...
{
    return "001 " + client + " :Welcome to the Internet Relay Network " + nick + "!" + host + CRLF;
}

std::string const RPL_YOURHOST(std::string const &client, std::string const &servname, std::string const &vers)
{
    return "002 " + client + " :Your host is " + servname + ", running version " + vers + CRLF;
}

std::string const RPL_CREATED(std::string const &client, std::string const &datetime)
{
    return "003 " + client + " :This server was created " + datetime + CRLF;
}
